...
ClassImp(TMugastPhysics)
  ...
  TMugastPhysics::TMugastPhysics() {
    ...
    m_DSSD_X_E_RAW_Threshold = 16400;
    //m_DSSD_Y_E_RAW_Threshold = 8200;
    m_DSSD_Y_E_RAW_Threshold = 16400;
...
//cout << "here_StarPreTreat" << endl;
  ClearPreTreatedData();
//cout << "here_AfterClearPreTreat" << endl;

int preTcount = 0;
//cout << "PreT -> " << preTcount << endl; preTcount++;
...
//cout << "mult=" << DSSDX_EMult << endl;
  for (unsigned int i = 0; i < DSSDX_EMult; ++i) {
//cout << "in" << endl;
//cout << "count? " << m_EventData->GetDSSDXEDetectorNbr(i) << endl;
...
//cout << m_EventData->GetDSSDXEDetectorNbr(i) << endl;
//cout << m_EventData->GetDSSDXEStripNbr(i) << endl;

//if(IsValidChannel(0, m_EventData->GetDSSDXEDetectorNbr(i),m_EventData->GetDSSDXEStripNbr(i))){
//	cout << 2 << endl;
//}
//cout << "end" << endl;
...
//cout << "====================================================" << endl;
//cout << "====================================================" << endl;
//cout << "here_NPLBuildPhysEvent" << endl;

  PreTreat();

//cout << "here_AfterPreTreat" << endl;
...
//cout << "here_IsValid" << endl;
//cout << "map size = " << m_XChannelStatus[telescope].size() << endl;  
//cout << "returns = " 
//     << *(m_XChannelStatus[telescope].begin() + channel - 1)
//     << endl;  
...
//    cout << "In the block, i = " << i << "/" << blocks.size() << endl;
...
//      cout << "disabled y " << v[0] << " " << v[1] << endl;
    ...
    //cout << endl << "//// MUGAST thresholds set to X = " << m_DSSD_X_E_Threshold << " & Y = " << m_DSSD_Y_E_Threshold << endl;
...
void TMugastPhysics::ReadConfiguration(NPL::InputParser parser) {
  ...
  for (unsigned int i = 0; i < blocks.size(); i++) {
    ...
    if (blocks[i]->HasTokenList(cart)) {
      ...
      //NumberofRing = blocks[i]->GetInt("NumRing");
      NumberofRing.push_back(blocks[i]->GetInt("NumRing"));
      //NumberofSector = blocks[i]->GetInt("NumSector");
      NumberofSector.push_back(blocks[i]->GetInt("NumSector"));
      AddTelescope(DetectorType[detectorNbr],Center);
      //AddTelescope(DetectorType[detectorNbr],Center,Thickness,NumRing, NumSector);
    }
...
void TMugastPhysics::AddParameterToCalibrationManager() {
  ...
  vector<double> standardX    = {-127, 127. / 16384.};
  vector<double> standardY    = {127, -127. / 16384.};
  //vector<double> standardSecondLayer  = {-127, 127. / 16384.};
  //vector<double> standardT    = {-1000, 1000. / 16384.};
...
//cout << "num detectors correct? -> " << m_NumberOfTelescope << endl;
...
//void TMugastPhysics::AddTelescope(MG_DetectorType type,TVector3 C_Center, double Thickness, int NumberofRing, int NumberofSector) {
  // To avoid warning
  m_NumberOfTelescope++;
  double Z = C_Center.Z();
...
//  int NumberofSector = 16 ; //Per detector, ( 4 in each Quad)
  ...
  double StripPitchSector = (Phi_Max-Phi_Min)/(NumberofSector.at(AddTelescopeCycles)) ; //radial strip spacing in rad 
  //double StripPitchRing = (R_Max-R_Min)/NumberofRing  ; // ring strip spacing in mm
  double StripPitchRing = (R_Max-R_Min)/NumberofRing.at(AddTelescopeCycles)  ; // ring strip spacing in mm
  ...
  for(int iQuad = 0; iQuad < NumberOfQuadrant ; iQuad++){
    ...
    for(int iRing = 0 ; iRing < NumberofRing.at(AddTelescopeCycles); iRing++){
      ...
      for(int iSector = 0 ; iSector < NumberofSector.at(AddTelescopeCycles) ; iSector++){
...
//cout << "charlietest - before resize" << endl;
...
//cout << "charlietest - after resize" << endl;
...
namespace MUGAST_LOCAL {
  ...
  double fDSSD_X_E(const TMugastData* m_EventData, const int& i) {
    ...
    //return CalibrationManager::getInstance()->ApplyCalibrationDebug(
