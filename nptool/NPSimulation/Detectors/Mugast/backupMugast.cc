...
namespace Mugast_NS{
  // Resolution (here expressed in terms of Sigma, NOT FWHM as with MUST2)
  const G4double SigmaTime    = 0.212765957; // = 500ps
  const G4double SigmaEnergy  = 0.013;       //0.019; 
//  const G4double TimeOffset   = 500         ;// 500 ns stop

  // Threshold
  const G4double EnergyThreshold = 1 * MeV;
  //const G4double EnergyThreshold = 0.0 * MeV;

  // Geometry
  //const G4double AluStripThickness = 0.4*micrometer; //CPx: reinstated for simulation
  const G4double SiliconThickness  = 500*micrometer; // Beaumel GRIT Project report, June 2019
