...
namespace Mugast_NS {
  // Resolution (here expressed in trems of sigma, NOT FWHM as with MUST2)
  const G4double SigmaTime = 0.212765957; // = 500ps
  const G4double SigmaEnergy = 0.013;     //
  ...
  ////////////////////////////////////////////////////////////////////////
  const G4double SiliconThickness = 500 * micrometer; // Beumel GRIT Project report, June 2019
  ////////////////////////////////////////////////////////////////////////
  ...
  const G4int NbQuadrant = 1;
  //const G4int NbrRingStrips = 16;
  //const G4int NbrSectorStrips = 16;
  //const G4double WaferOutterRadius = 100*mm;		//50*mm;
  //const G4double WaferInnerRadius = 25*mm;		//23*mm;
  const G4double WaferOutterRadius = 50*mm;		//50*mm;
  const G4double WaferInnerRadius = 12.5*mm;		//23*mm;
  //const G4double WaferThickness = thickCPx*micrometer;	//500
  const G4double WaferRCut = 100*mm;
  //const G4double WaferRCut = 45.5*mm;
  //const G4double ActiveWaferOutterRadius = 99*mm;	//48*mm;
  //const G4double ActiveWaferInnerRadius = 26*mm;	//24*mm;
  const G4double ActiveWaferOutterRadius = 50*mm;	//48*mm;
  const G4double ActiveWaferInnerRadius = 12.5*mm;	//24*mm;
  const G4double ActiveWaferRCut = 99*mm;
  //const G4double ActiveWaferRCut = 44.5*mm;
  //const G4double PCBPointsX[8] = {-80, 80, 120, 120, 80, -80, -120, -120};
  //const G4double PCBPointsY[8] = {120, 120, 80, -80, -120, -120, -80, 80};
  const G4double PCBPointsX[8] = {-40, 40, 60, 60, 40, -40, -60, -60};
  const G4double PCBPointsY[8] = {60, 60, 40, -40, -60, -60, -40, 40};


  //const G4double PCBThickness = 30. * mm;
  //const G4double PCBThickness = 5. * mm;
...
Mugast::Mugast() {
  ...
  m_TrapezoidScorer = 0;
  m_AnnularScorer = 0;
  m_AnnularScorerBack = 0;
  m_SquareDetector = 0;
  m_TrapezoidDetector = 0;
  m_AnnularDetector = 0;
  m_AnnularDetectorBack = 0;
...
cout << "END OF ADDDET" << endl;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4LogicalVolume* Mugast::BuildSquareDetector() {
cout << "START OF BUILDSQUARE" << endl;
  if (!m_SquareDetector) {
...
cout << "END OF BUILDSQUARE" << endl;
  return m_SquareDetector;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4LogicalVolume* Mugast::BuildTrapezoidDetector() {
cout << "START OF BUILDTRAP" << endl;
  if (!m_TrapezoidDetector) {
    ...
    G4VisAttributes* TrapezoideVisAtt = new G4VisAttributes(G4Colour(0.25, 0.0, 0.34));
    //G4VisAttributes* TrapezoideVisAtt = new G4VisAttributes(G4Colour(0.3, 0.3, 0.3));
    TrapezoideVisAtt->SetForceWireframe(true);
    //TrapezoideVisAtt->SetForceWireframe(false);
    ...
    /// Visualisation of FirstStage Strip
    //G4VisAttributes* FirstStageVisAtt = new G4VisAttributes(G4Colour(0.3, 0.3, 0.3));
    G4VisAttributes* FirstStageVisAtt = new G4VisAttributes(G4Colour(0.33, 0.00, 0.47));
    logicFirstStage->SetVisAttributes(FirstStageVisAtt);

    m_TrapezoidDetector = logicTrapezoid;
  }

cout << "END OF BUILDTRAP" << endl;
  return m_TrapezoidDetector;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4LogicalVolume* Mugast::BuildAnnularDetector() {
cout << "START OF BUILDANNULAR" << endl;
  
  double thicknessNow = WaferThickness[stage];
  
  const G4double PCBThickness = thicknessNow * mm;
  
  //double thicknessNow = 0;
  //if(!firstStagePassed){
  //  thicknessNow = WaferThickness[0];
  //} else {
  //  thicknessNow = WaferThickness[1];
  //}
...
//////////////////////////////////////////////////////
  ...
  G4Tubs* solidWaferBase =
      ...
      new G4Tubs("Wafer", WaferInnerRadius, WaferOutterRadius, 0.5 * thicknessNow, 0 * deg, 360 * deg);
  ...
  G4Tubs* solidActiveWaferBase = new G4Tubs("ActiveWafer", ActiveWaferInnerRadius, ActiveWaferOutterRadius,
                                            ...
                                            0.5 * thicknessNow, 0 * deg, 360 * deg);
  ...
  if(stage==0){
    ...
    // Set Silicon strip sensible
    logicActiveWafer->SetSensitiveDetector(m_AnnularScorerBack);
  }

  //firstStagePassed = true;
  stage++;

cout << "END OF BUILDANNULAR" << endl;
...
cout << "START OF READCONFIG " << endl;
  ...
  vector<string> annular = {"DetectorNumber", "Center","Thickness","NumRing","NumSector"};
  ...
  for (unsigned int i = 0; i < blocks.size(); i++) {
    ...
    if (blocks[i]->HasTokenList(cart) && (shape == "Square" || shape == "Trapezoid")) {
      ...
      //SiliconThickness = blocks[i]->GetDouble("Thickness","mm");
      ...
      WaferThickness.push_back( blocks[i]->GetDouble("Thickness","mm") );
      //NbrRingStrips = blocks[i]->GetInt("NumRing");
      NbrRingStrips.push_back( blocks[i]->GetInt("NumRing") );
      //NbrSectorStrips = blocks[i]->GetInt("NumSector");
      NbrSectorStrips.push_back( blocks[i]->GetInt("NumSector") );
      AddDetector(DetectorNumber, shape, A);
      //AddDetector(DetectorNumber, shape, A, Thickness, NumRing, NumSector);
    }
...
cout << "START OF CONSTRUCTDETECTOR" << endl;
...
cout << "END OF CONSTRUCTDETECTOR" << endl;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// Add Detector branch to the EventTree.
// Called After DetecorConstruction::AddDetector Method
void Mugast::InitializeRootOutput() {
cout << "START OF INITILIZEROOTOUT" << endl;
...
cout << "END OF INITILIZEROOTOUT" << endl;
}
...
//cout << "START OF READSENSITIVE" << endl;
  m_Event->Clear();
  ...
  for (unsigned int i = 0; i < sizeBack; i++) {
    ...
    if (Energy > EnergyThreshold) {
      ...
      m_Event->SetDSSDYE(MG_DetectorType::MG_NOCHANGE, DetNbr, MUGAST_MAP::ReverseAnnularY[StripBack - 1],
                         //NPL::EnergyToADC(Energy, 0, 63, 8192, 0));
                         NPL::EnergyToADC(Energy, 0, 127, 16384, 0));
                         //ADC);
...
//////////////////////////////
  ...
  // Annular BACKING LAYERS!!!!!!!!!!!!!!!11
  DSSDScorers::PS_Annular* AnnularScorerBack = (DSSDScorers::PS_Annular*)m_AnnularScorerBack->GetPrimitive(0);

  // Loop on the Annular map
  sizeFront = AnnularScorerBack->GetRingMult();
  sizeQuadrant = AnnularScorerBack->GetQuadrantMult();

  for (unsigned int i = 0; i < sizeFront; i++) {

    double Energy = RandGauss::shoot(AnnularScorerBack->GetEnergyRing(i), SigmaEnergy);

    if (Energy > EnergyThreshold) {
      double Time = RandGauss::shoot(AnnularScorerBack->GetTimeRing(i), SigmaTime);
      unsigned int DetNbr = AnnularScorerBack->GetDetectorRing(i);
      unsigned int StripFront = AnnularScorerBack->GetStripRing(i);

      // Check for associated Quadrant strip
      int StripQuadrant = 0;
      for (unsigned int q = 0; q < sizeQuadrant; q++) {
        if (AnnularScorerBack->GetDetectorQuadrant(q) == DetNbr) {
          StripQuadrant = AnnularScorerBack->GetStripQuadrant(q) - 1;
          break;
        }
      }
      //StripFront = StripFront + StripQuadrant * NbrRingStrips;
  ...
  sizeBack = AnnularScorerBack->GetSectorMult();
  for (unsigned int i = 0; i < sizeBack; i++) {

    double Energy = RandGauss::shoot(AnnularScorerBack->GetEnergySector(i), SigmaEnergy);

    if (Energy > EnergyThreshold) {
      double Time = RandGauss::shoot(AnnularScorerBack->GetTimeSector(i), SigmaTime);
      int DetNbr = AnnularScorerBack->GetDetectorSector(i);
      int StripBack = AnnularScorerBack->GetStripSector(i);
...
//cout << "END OF READSENSITIVE" << endl;
...
cout << "START OF INITSCORERS" << endl;
  ...
  m_AnnularScorerBack = CheckScorer("AnnularScorerBack", already_exist);

cout << "InitScore 1" << endl;
  if (already_exist)
    return;

cout << "InitScore 2" << endl;

  // Otherwise the scorer is initialised
  G4VPrimitiveScorer* SquareScorer =
      new DSSDScorers::PS_Rectangle("MugastSquareScorer", 1, SquareBase, SquareHeight, 128, 128);

cout << "InitScore 3" << endl;
...
cout << "InitScore 4" << endl;

  G4VPrimitiveScorer* AnnularScorer;
  G4VPrimitiveScorer* AnnularScorerBack;
if(NbrRingStrips.size()>0){
cout << " || DOING ANNULAR DETECTOR SIMULATION || "<< endl;
  //G4VPrimitiveScorer* AnnularScorer =
  AnnularScorer =
      new DSSDScorers::PS_Annular("MugastAnnularScorer", 2, ActiveWaferInnerRadius, ActiveWaferOutterRadius,
                                  ...
                                  +8 * 22.5 * deg, NbrRingStrips.at(0), NbrSectorStrips.at(0), NbQuadrant);


cout << "InitScore 5" << endl;

  //G4VPrimitiveScorer* AnnularScorerBack =
  AnnularScorerBack =
      new DSSDScorers::PS_Annular("MugastAnnularScorerBack", 2, ActiveWaferInnerRadius, ActiveWaferOutterRadius,
                                  -8 * 22.5 * deg, // MUST2 campaign 2009, See: Phd Sandra Giron
                                  +8 * 22.5 * deg, NbrRingStrips.at(1), NbrSectorStrips.at(1), NbQuadrant);

} else {
//Not doing a GRIT annula simulation, therefore use standard so as to not break code
cout << " || STANDARD SIMULATION, NOT FOR GRIT ANNULAR || "<< endl;

  //G4VPrimitiveScorer* AnnularScorer =
  AnnularScorer =
      new DSSDScorers::PS_Annular("MugastAnnularScorer", 2, ActiveWaferInnerRadius, ActiveWaferOutterRadius,
                                  -8 * 22.5 * deg, // MUST2 campaign 2009, See: Phd Sandra Giron
                                  +8 * 22.5 * deg, 32, 32, NbQuadrant);
...
cout << "InitScore 6" << endl;
  ...
  G4VPrimitiveScorer* InteractionAB = new InteractionScorers::PS_Interactions("InteractionAB", ms_InterCoord, 0);
  ...
  m_AnnularScorerBack->RegisterPrimitive(AnnularScorerBack);
  m_AnnularScorerBack->RegisterPrimitive(InteractionAB);


cout << "InitScore 7" << endl;
  ...
  G4SDManager::GetSDMpointer()->AddNewDetector(m_AnnularScorerBack);
cout << "END OF INITSCORERS" << endl;
}
